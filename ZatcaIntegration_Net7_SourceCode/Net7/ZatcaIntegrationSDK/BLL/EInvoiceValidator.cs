    ...
    public class EInvoiceValidator : IDisposable
    {
        ...
        private bool ValidateSignature(XmlDocument xmlDoc, ref string errorMessage, string certificateContent)
        {
            try
            {
                ...
                byte[] arrCertificate = Convert.FromBase64String(CERTIFICATE);
                ...
                try
                {
                    ...
                    X509Certificate2 objX509Certificate = new X509Certificate2(arrCertificate);
                ...
                try
                {
                    byte[] certBytes = Convert.FromBase64String(certificateContent);
                    certificate = new X509Certificate2(certBytes);
                }
                catch
                {
                    errorMessage = "Invalid Certificate in validate function";
                    return false;
                }
