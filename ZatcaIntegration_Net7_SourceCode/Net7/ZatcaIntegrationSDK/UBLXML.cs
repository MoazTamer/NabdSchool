...
namespace ZatcaIntegrationSDK
{
    ...
    public class UBLXML : IDisposable
    {
        ...
        /// </returns>
        public Result GenerateInvoiceXML(Invoice inv, string Directorypath = "", bool savexml = true, bool validatexml = true)
        {
        ...
        private void SignDocument(string Directorypath, string xmldocument, string returnsignedxmldir, string returnnormalxmldir, Invoice inv, ref Result result)
        {
            try
            {
                ...
                if (inv.legalMonetaryTotal.PayableRoundingAmount > 1)
                {
                    result.IsValid = false;
                    result.ErrorMessage = "Error in PayableRoundingAmount Kindly Check xml totals.";
                    return;
                }
        ...
        //        DateTime.TryParseExact("2025-05-01", SettingsParams.allDatesFormats, CultureInfo.InvariantCulture, DateTimeStyles.None, out trailenddate);
     ...
     .Select(
         l => new
         {
            ...
            foreach (var grp in LinesGroupedwithvat)
            {
                ...
                {
                    subtotal.taxCategory.ID = grp.VatCategory.Trim();
                    subtotal.taxCategory.Percent = grp.VatPercent;
                    subtotal.TaxableAmount = grp.LineExtensionAmount;
                    subtotal.TaxAmount = grp.LineExtensionAmount * (grp.VatPercent / 100);
                    subtotal.taxCategory.TaxExemptionReasonCode = !string.IsNullOrEmpty(grp.TaxSubtotal.taxCategory.TaxExemptionReasonCode) ? grp.TaxSubtotal.taxCategory.TaxExemptionReasonCode.Trim().ToUpper() : "";
                    subtotal.taxCategory.TaxExemptionReason = !string.IsNullOrEmpty(grp.TaxSubtotal.taxCategory.TaxExemptionReason) ? grp.TaxSubtotal.taxCategory.TaxExemptionReason.Trim() : "";
                    ...
                    subtaxkeys.Add(grp.VatPercent.ToString("0.00") + grp.VatCategory.Trim(), subtotal);
                }
            }
            ...
            foreach (AllowanceCharge allowance in inv.allowanceCharges)
            {
                ...
                if (allowanceamount > 0 && allowance.ChargeIndicator == false)
                {
                    ...
                    {
                        TaxSubtotal subtotal = new TaxSubtotal();
                        subtotal.taxCategory.ID = allowance.taxCategory.ID.Trim();
                        subtotal.taxCategory.Percent = allowance.taxCategory.Percent;
                        subtotal.TaxableAmount = allowanceamount * -1;
                        subtotal.TaxAmount = allowancevat * -1;
                        subtotal.taxCategory.TaxExemptionReasonCode = !string.IsNullOrEmpty(allowance.taxCategory.TaxExemptionReasonCode) ? allowance.taxCategory.TaxExemptionReasonCode.Trim().ToUpper() : "";
                        subtotal.taxCategory.TaxExemptionReason = !string.IsNullOrEmpty(allowance.taxCategory.TaxExemptionReason) ? allowance.taxCategory.TaxExemptionReason.Trim() : "";
                        subtaxkeys.Add(allowance.taxCategory.Percent.ToString("0.00") + allowance.taxCategory.ID.Trim(), subtotal);
                    }
                }
                if (chargeamount > 0 && allowance.ChargeIndicator == true)
                {
                    ...
                    else
                    {
                        ...
                        subtotal.taxCategory.ID = allowance.taxCategory.ID.Trim();
                        subtotal.taxCategory.Percent = allowance.taxCategory.Percent;
                        subtotal.TaxableAmount = chargeamount;
                        subtotal.TaxAmount = chargevat;
                        subtotal.taxCategory.TaxExemptionReasonCode = !string.IsNullOrEmpty(allowance.taxCategory.TaxExemptionReasonCode) ? allowance.taxCategory.TaxExemptionReasonCode.Trim().ToUpper() : "";
                        subtotal.taxCategory.TaxExemptionReason = !string.IsNullOrEmpty(allowance.taxCategory.TaxExemptionReason) ? allowance.taxCategory.TaxExemptionReason.Trim() : "";
                        subtaxkeys.Add(allowance.taxCategory.Percent.ToString("0.00") + allowance.taxCategory.ID.Trim(), subtotal);
                    }
            ...
            "<cbc:TaxAmount currencyID='" + inv.DocumentCurrencyCode + "'>" + Math.Round(sub.TaxAmount, 2, MidpointRounding.AwayFromZero).ToString("0.00") + "</cbc:TaxAmount>");
                ...
                "<cbc:ID>" + sub.taxCategory.ID.Trim().ToUpper() + "</cbc:ID>" +
                ...
                if (sub.taxCategory.ID == "O" || sub.taxCategory.ID == "E" || sub.taxCategory.ID == "Z")
                {
                    ...
                    {
                        subtotaltext.Append("<cbc:TaxExemptionReasonCode>" + Utility.ReplaceXMLSpecialCharacters(sub.taxCategory.TaxExemptionReasonCode.Trim()).ToUpper() + "</cbc:TaxExemptionReasonCode>");

                    }
                    if (!string.IsNullOrEmpty(sub.taxCategory.TaxExemptionReason))
                    {
                        subtotaltext.Append("<cbc:TaxExemptionReason>" + Utility.ReplaceXMLSpecialCharacters(sub.taxCategory.TaxExemptionReason.Trim()) + "</cbc:TaxExemptionReason>");
                    ...
                    else
                    {
                        if (!string.IsNullOrEmpty(sub.taxCategory.TaxExemptionReasonCode))
                        {
                            ...
                            string taxexemptionreason = GetTaxExemptionReason(Utility.ReplaceXMLSpecialCharacters(sub.taxCategory.TaxExemptionReasonCode.Trim()).ToUpper());
            ...
            if (inv.DocumentCurrencyCode.Trim().ToUpper() != "SAR")
            {
                if (inv.CurrencyRate <= 0)
                {
                    error += "\n";
                    error += "If document currency code not equal SAR , Currency rate must be greater than Zero ";
                }
        ...
        private void RepairTaxCategory(InvoiceLine invline)
        {
            ...
            {
                //tax category code
                if (string.IsNullOrEmpty(invline.item.classifiedTaxCategory.ID))
                {
                    invline.item.classifiedTaxCategory.ID = invline.taxTotal.TaxSubtotal.taxCategory.ID;
                }
                if (string.IsNullOrEmpty(invline.taxTotal.TaxSubtotal.taxCategory.ID))
                {
        ...
        #endregion
        #region ExtraMethods
        public bool SaveXml(string EncodedInvoice, string Directorypath)
        {
            try
            {
                if (!string.IsNullOrEmpty(EncodedInvoice) && !string.IsNullOrEmpty(Directorypath))
                {
                    var doc = new XmlDocument();
                    doc.PreserveWhitespace = true;
                    string SingedXML = Utility.Base64Dencode(EncodedInvoice);
                    ...
                    string signedxmlfilename = Directorypath + xmlfile;
                    if (this.SaveXML)
                    {
