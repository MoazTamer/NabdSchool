...
var KTList = function () {
    ...
    var initTable = function () {
        var CustomerData_ID = $("#CustomerData_ID").val();
        ...
        datatable = $(table).DataTable({
            ...
            processing: true,
            "ajax": {
                "url": "/Customer_Data/GetDetail?CustomerID=" + CustomerData_ID + "&FromDate=" + FromDate + "&ToDate=" + ToDate
            },
            "language": {
                "search": "البحث ",
            ...
            'order': [],
            'columnDefs': [
                { targets: [3, 4, 5, 6, 7], orderable: false, searchable: false, className: "text-center" },
                { targets: [0, 1], orderable: false, className: "text-center" },
                { targets: [2], orderable: false },
                { targets: [6], visible: false },
                { targets: [3, 4, 5], render: $.fn.dataTable.render.number('', '.', 2, '') }
            ],
            "columns": [
                { "data": null },
                {
                ...
                { "data": "statementType" },
                {
                    "data": "id",
                    "render": function (data, type, row) {
                        if (row.statementType == "pay") {
                            return `
                                <a onclick=Detail("/Customer_Data/DetailPayment/${data}") style="cursor:pointer">
                                   <i class="fa-solid fa-magnifying-glass text-primary fs-1"></i>
                                </a>
                           `;
                        }
                        else if (row.statementType == "sale") {
                            return `
