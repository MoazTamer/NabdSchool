...
var KTList = function () {
    ...
    var initTable = function () {
        var SaleID = $("#SalesInvoice_ID").val();
        // Init datatable --- more info on datatables: https://datatables.net/manual/
        datatable = $(table).DataTable({
            responsive: true,
            processing: true,
            "ajax": {
                "url": "/Sales/GetInvoiceProduct/" + SaleID
            },
            "language": {
                "search": "البحث ",
            ...
            'order': [],
            'columnDefs': [
                { targets: [0, 6, 7, 8, 9, 10, 11, 13], orderable: false, searchable: false, className: "text-center border border-gray-500" },
                { targets: [1, 4, 5], className: "text-center border border-gray-500" },
                { targets: [2, 3, 9, 12], visible: false }
            ],
            "columns": [
                { "data": null },
                ...
                },
                {
                    ...
                    "render": function (data, type, row) {
                        return `
                                <input type="text" class="form-control form-control-solid text-center price" value="${$.fn.dataTable.render.number('', '.', 2, '').display(data)}" />
                           `;
                    }
                },
                {
                ...
                { "data": "productBarcode_BuyPrice" },
                {
                    ...
                    "render": function (data, type, row) {
                        return `
                                <input type="text" class="form-control form-control-solid text-center priceVat" value="${$.fn.dataTable.render.number('', '.', 2, '').display(data)}" />
                           `;
                    }
                },
                {
                ...
                { "data": "salesInvoiceDetail_ID" },
                {
                    "data": "salesInvoiceDetail_ID",
                    "render": function (data, type, row) {
                        if (row.salesInvoiceDetail_ID == 0) {
                            return `
                           ...
                           `;
                            ...
                            return `
                                <a onclick=Delete("/Sales/DeleteDetail/${data}") style="cursor:pointer">
                                   <i class="fa-solid fa-trash-can text-danger fs-1"></i>
                                </a>
                           `;
                        }
                ...
                if (parseFloat($("#ProductBarcode_Count").val()) == 0) {
                    ...
                    if ($("#SalesInvoice_BillType").val() == "فاتورة مرتجعات") {
                        if ($("#SalesInvoice_Refrence").val() == "" || $("#SalesInvoice_Refrence").val() == null) {
                            ...
                            if ($("#SalesInvoice_Refrence").val() == "0") {
                                datatable.row
                                    .add({
                                ...
                                axios.get('/Sales/Barcode_KeyPress?Barcode=' + $("#ProductBarcode_Code").val() + '&BillType=' + $("#SalesInvoice_BillType").val() + '&Refrence=' + $("#SalesInvoice_Refrence").val())
                                    .then(function (response) {
                                        if (response.data.isValid == true) {
                                            //$("#ProductBarcode_Code").val('');
                                            $(datatable.row.add(response.data.data).draw().node()).find('td').eq(1).addClass('newItem');
...
function SaveBill(type) {
    ...
    if ($('tbody').length > 0) {
        ...
        sale.SalesInvoice_ID = $("#SalesInvoice_ID").val();
        ...
        $('tbody tr').each(function (i) {
            ...
            obj.SalesInvoiceDetail_ID = datatable.cell(i, 10).data();
        ...
        $.post('/Sales/Edit', { modelSales_Invoice: sale, print: type, afterVat: sale.SalesInvoice_TotalAfterVat }, function (data) {
