...
var KTList = function () {
    ...
    var initTable = function () {
        var ProductTitle_ID = $("#ProductOfferTitle_ID").val();
        // Init datatable --- more info on datatables: https://datatables.net/manual/
        datatable = $(table).DataTable({
            responsive: true,
            processing: true,
            "ajax": {
                "url": "/Product_Title/GetOfferBarcode?ProductTitle_ID=" + ProductTitle_ID
            },
            "language": {
                "search": "البحث ",
            ...
            "columns": [
                ...
                { "data": "productOffer_BuyPrice" },
                { "data": "productBarcode_Total" },
                { "data": "productOffer_ID" },
                {
                    "data": "productOffer_ID",
                    "render": function (data, row, type) {
                        if (data == 0) {
                           ...
                           `;
                            ...
                            return `
                                <a onclick=Delete("/Product_Title/Offer_Barcode_Delete/${data}") style="cursor:pointer">
                                   <i class="fa-solid fa-trash-can text-danger fs-1"></i>
                                </a>
                           `;
                        }
            ...
            if (status == 'Valid') {
                datatable.row
                    .add({
                        ...
                        "productOffer_BuyPrice": $("#ProductBarcode_BuyPrice").text(),
...
function SaveBill() {
    ...
    if ($('tbody').length > 0) {
        ...
        offer.ProductOfferTitle_ID = $("#ProductOfferTitle_ID").val();
        ...
        $('tbody tr').each(function (i) {
            ...
            obj.ProductOffer_ID = $(datatable.cell(i, 8).node()).text();
        ...
        $.post('/Product_Title/Offer_Edit', { modelProduct_Offer: offer }, function (data) {
...
function DeleteBill() {
    ...
    var Title_ID = $("#ProductOfferTitle_ID").val()
    $.post('/Product_Title/Offer_Delete/' + Title_ID, function (data) {
        if (data.isValid == true) {
            swal.fire({
                title: data.title,
                text: data.message,
