...
var KTList = function () {
    ...
    var initTable = function () {
        var BuyID = $("#BuyInvoice_ID").val();
        // Init datatable --- more info on datatables: https://datatables.net/manual/
        datatable = $(table).DataTable({
            responsive: true,
            processing: true,
            "ajax": {
                "url": "/Buy/GetInvoiceProduct/" + BuyID
            },
            "language": {
                "search": "البحث ",
            ...
            'order': [],
            'columnDefs': [
                { targets: [0, 4, 5, 6, 7, 8, 9, 10], orderable: false, searchable: false, className: "text-center" },
            ...
            "columns": [
                ...
                { "data": "buyInvoiceDetail_ID" },
                {
                    "data": "buyInvoiceDetail_ID",
                    "render": function (data, type, row) {
                        if (row.buyInvoiceDetail_ID == 0) {
                            return `
                           ...
                           `;
                            ...
                            return `
                                <a onclick=Delete("/Buy/DeleteDetail/${data}") style="cursor:pointer">
                                   <i class="fa-solid fa-trash-can text-danger fs-1"></i>
                                </a>
                           `;
                        }
...
KTUtil.onDOMContentLoaded(function () {
    ...
    //$("#lblVat").text("الضريبة " + $("#BuyInvoice_VatValue").val() + " %");
    GetVendorData();
    ChangeBillType();
    FillUnit()
    KTList.init();
...
function SaveBill(type) {
    ...
    if ($('tbody').length > 0) {
        ...
        buy.BuyInvoice_ID = $("#BuyInvoice_ID").val();
        ...
        buy.BuyInvoice_Total = $("#BuyInvoice_TotalAfterVat").text();
        ...
        $('tbody tr').each(function (i) {
            ...
            obj.BuyInvoiceDetail_ID = datatable.cell(i, 9).data();
        ...
        $.post('/Buy/Edit', { modelBuy_Invoice: buy, print: type }, function (data) {
