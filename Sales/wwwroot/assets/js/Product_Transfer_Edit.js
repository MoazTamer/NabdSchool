...
var KTList = function () {
    ...
    var initTable = function () {
        var TransferID = $("#ProductTransfer_ID").val();
        // Init datatable --- more info on datatables: https://datatables.net/manual/
        datatable = $(table).DataTable({
            responsive: true,
            processing: true,
            "ajax": {
                "url": "/Product_Transfer/GetTransferProduct/" + TransferID
            },
            "language": {
                "search": "البحث ",
            ...
            "columns": [
                ...
                { "data": "productTransferDetail_ID" },
                {
                    "data": "productTransferDetail_ID",
                    "render": function (data, type, row) {
                        if (row.productTransferDetail_ID == 0) {
                            return `
                           ...
                           `;
                            ...
                            return `
                                <a onclick=Delete("/Product_Transfer/DeleteDetail/${data}") style="cursor:pointer">
                                   <i class="fa-solid fa-trash-can text-danger fs-1"></i>
                                </a>
                           `;
                        }
...
function FillToBranch() {
    ...
    axios.get('/Product_Transfer/GetToBranch?FromBranchID=' + FromBranchID)
        .then(function (response) {
            ...
            $("#ToBranchID").val($("#ToBranchEdit").val());
        })
        .catch(function (error) {
            swal.fire({
                title: "التحويلات بين الفروع",
...
function SaveBill(type) {
    ...
    if ($('tbody').length > 0) {
        ...
        transfer.ProductTransfer_ID = $("#ProductTransfer_ID").val();
        ...
        $('tbody tr').each(function (i) {
            ...
            obj.ProductTransferDetail_ID = datatable.cell(i, 5).data();
        ...
        $.post('/Product_Transfer/Edit', { modelProduct_Transfer: transfer }, function (data) {
